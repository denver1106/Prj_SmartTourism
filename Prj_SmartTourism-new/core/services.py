...
class SmartTourismService:
    ...
    def get_weather_context(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if "rain" in raw_cond or "drizzle" in raw_cond:
                ...
                # Gộp bão vào mưa để gợi ý món nóng
                weather_type = "rainy"
    ...
    def process_user_input(
        ...
        # Chú ý: nếu DataManager chưa có get_user_history() thì bạn có thể
        # dùng get_user_history_full() thay thế.
        try:
            user_history = data_manager.get_user_history(user_id)  # type: ignore[attr-defined]
        except AttributeError:
            user_history = data_manager.get_user_history_full(user_id)
        ...
        if recommendations:
            ...
            # Nếu muốn log lại có thể dùng:
            # data_manager.update_user_history(user_id, top_item["id"], action="auto_suggest")
    ...
    def get_restaurant_detail(
        ...
        """
        Lấy thông tin chi tiết 1 quán để hiển thị ở Result - Detail Page.

        - Gọi xuống DataManager.get_restaurant_by_id (đã chuẩn hóa object).
        - Có thể truyền kèm user_lat/user_lon để tính khoảng cách.
        """
        return data_manager.get_restaurant_by_id(
            restaurant_id, user_lat=user_lat, user_lon=user_lon
        )
    ...
    def build_detail_page_context(
        ...
        """
        Helper dùng cho Controller (app.py) để chuẩn bị toàn bộ dữ liệu
        cho trang Result - Detail Page:

        - Thông tin quán (restaurant)
        - Danh sách quán gần đó (nearby, nếu có tọa độ user)
        - Thông tin thời tiết tại vị trí user (weather, nếu có tọa độ)
        - Tự động cập nhật lịch sử khi user xem chi tiết quán.
        """
        restaurant = self.get_restaurant_detail(
        ...
        if not restaurant:
            return {"status": "not_found", "restaurant": None}

        nearby: List[Dict[str, Any]] = []
        weather_ctx: Optional[Dict[str, Any]] = None

        # Nếu có vị trí user thì gợi ý thêm vài quán gần đó
        if user_lat is not None and user_lon is not None:
            try:
                nearby = data_manager.get_restaurants_near_user(
                    user_lat, user_lon, radius_km=3
                )
                # Loại bỏ chính quán hiện tại ra khỏi danh sách nearby
                nearby = [r for r in nearby if r["id"] != restaurant_id]
            except Exception as e:
                print("[SmartTourismService] Nearby error:", e)

            try:
                weather_ctx = self.get_weather_context(user_lat, user_lon)
            except Exception:
                weather_ctx = None
        ...
        if user_id is not None:
            try:
                data_manager.update_user_history(
                    user_id=user_id,
                    rid=restaurant_id,
                    action="view_detail",
                )
            except Exception as e:
                print("[SmartTourismService] update_user_history error:", e)
